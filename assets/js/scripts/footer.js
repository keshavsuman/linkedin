...
(function($, Drupal) {
  ...
  Drupal.behaviors.accordion = {
    attach: function(context) {
      $('.paragraph--type--accordion-item .accordion-body', context).once('accordionBehavior').each(function() {
      ...
      $('.accordion-title').on('click', function(event) {
        ...
        $(this).find(".accordion-toggle").toggleClass("fa-angle-down").toggleClass("fa-angle-up")
      })
    }
  }
})(jQuery, Drupal);
  ...
  Drupal.behaviors.hamburger_menu = {
    attach: function(context) {
      ...
      var $toggle_mob_link = $('[data-toggle=mobile] .toggle-icon', context),
        ...
        $toggle_overlay = $('body > .overlay', context),
      ...
      $(document).on('click', function(e) {
        if ($(e.target).closest($toggle_mob_ul).length === 0) $toggle_li.removeClass('is-hover');
        if ($(e.target).closest($toggle_drop).length === 0);
      });
  ...
  Drupal.behaviors.MegaMenuDropDown = {
    attach: function(context) {
      function countMegamenuWidth() {
        var mainDropMenu = $("#block-maindropdown .navbar-we-mega-menu > .container-fluid > ul > li.dropdown-menu", context);
        mainDropMenu.each(function() {
          var menuWidth = $(window).width() - ($(this).offset().left),
            ...
            dropMenuCol = $(this).find('.we-mega-menu-submenu:first > .we-mega-menu-submenu-inner > .we-mega-menu-row > .we-mega-menu-col', context),
...
(function($, Drupal) {
  ...
  Drupal.behaviors.navigationMobile = {
    attach: function(context) {
      $("#block-mainnavigation", context).once().each(function() {
        $("<div class='mobile-nav'><select id='mobile-select' /></div>").appendTo("#block-mainnavigation");
...
(function($, Drupal) {
  ...
  Drupal.behaviors.insights = {
    attach: function(context) {
      $('#edit-field-insights-category-target-id-1, #edit-field-press-category-target-id-1', context).once().prepend($('<option>', {
        value: 'All',
        text: 'Filter By Category',
        selected: true
      }));
      $('.view-insights-by-category .form-submit').click(function(event) {
        var $input = $('.view-insights-by-category .form-item-keys input');
      ...
      try {
        var myText = $("#block-mainnavigation > .arrows > li", context).find("a.is-active").text();
...
(function($, Drupal) {
  ...
  Drupal.behaviors.animations = {
    attach: function(context) {
      ...
      if ($circleAnimateMenu.length) $circleAnimateMenu.each(function() {
        var $menu = $(this),
          $link = $menu.find('li'),
...
(function($, Drupal) {
  ...
  Drupal.behaviors.referrer = {
    attach: function(context) {
      var $floating_block = $('.js-floating-block', context),
        $general_header = $('.general-header', context),
        $dark_block = $('.js-is-dark-block', context),
        $root = $('html, body', context),
        $form_block = $('#bfm-contact-us-form', context),
        $footer_block = $('.footer-main', context),
        $relative_block = $('#bfm-contact-us-form, .footer-main', context)

      function $floating_en() {
        function $floating_en_init() {
          var $general_header_h = $general_header.height();
          if ($floating_block.offset().top > $general_header_h) $floating_block.addClass('is-visible')
        };
        $floating_en_init();
        $(window).on('resize', $floating_en_init)
      };
      if ($floating_block.length) {
        if ($general_header.length) {
          $floating_en();
          $general_header.viewportChecker({
            classToAdd: 'is-visible',
            ...
            callbackFunction: function(elem, action) {
              if (action === 'remove') {
                $floating_block.addClass('is-visible')
              } else $floating_block.removeClass('is-visible')
            }
          })
        };
        if ($footer_block.length) $footer_block.viewportChecker({
          repeat: true,
          offset: 400,
          callbackFunction: function(elem, action) {
            if (action === 'add') {
              $floating_block.addClass('is-relative')
            } else $floating_block.removeClass('is-relative')
          }
        })
      };
      var $space_size, $toolbar = $('#toolbar-administration', context);
      if ($toolbar.length) {
        $space_size = 230
      } else $space_size = 120;
      $('a[data-referrer="true"]', context).on('click', function(e) {
        e.preventDefault();
        if ($form_block.length) $root.animate({
          scrollTop: $form_block.offset().top - $space_size
        }, 1e3)
      })
    }
  }
