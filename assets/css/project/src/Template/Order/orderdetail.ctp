...
<div class="row" style="border:1px solid;">
 <div class="col-3">
 <img style="max-width:225px;max-height: 250px;padding:8%;" src="<?php echo "http://resellermantra.com/image/".$o['pic'];   ?>"/>
  ...
  if($o['payment_method']==1)
	...
	// supplier field 
	$orignal_value_total=$s_product_price=(int)$o['orignal_value']*$qty;
	$tcs=(1/ 100) * $s_product_price;
	...
	$show_pen=0;
    // reseller fields
	$r_product_price=((int)$o['sale_base']-$total_shipping)-$my_earning; 
	$cod_amount=(int)$o['cod_amount'];
	$discount_amount=(int)$o['discount_amount'];
	  $buyer_pay=(int)$o['sale_base'];
	if($order_status==11 || $order_status==4 || $order_status==5 || $order_status==7)
	{
		// supplier field 
		$orignal_value_total=0;
		$tcs=0;;
  ...
  ?>
  <p>
    <span class="btn btn-primary" style="color:<?php echo $p['color'];?>;font-weight:bold;background:<?php echo $o['s_color'];?>"><?php echo $o['show_status'];?></span><br/>
  <b style="font-size: 18px;font-weight: bold;"><?php echo $o['name'];?></b> <br/>
  Order Item id : <?php echo $o['item_order_id'];?><br/>
  Order Date : <?php echo date('d-m-Y h:i A',strtotime($o['order_date']));?></br>
   Qty : <?php echo (int)$o['qty'];?><br/>
   Size : <?php echo $o['size_name'];?><br/>
   Order Type : <?php echo $order_type;?><br/>
   <?php if($o['feedback_user']){ ?>
	<b>FeedBack User :</b> <?php echo $o['feedback_user']; ?></br>
	Product Rating : <?php echo $o['rating_by_user']; ?>
	<?php } ?>
 ...
 <div class="col-md-4">
 <h4>Buyer Detail</h4>
  <?php echo $cus_address['name'].",".$cus_address['contact']; ?></br>
	<?php echo $cus_address['address'].",".$cus_address['address2']; ?></br>
	<?php echo $cus_address['city'].",".$cus_address['state'].",".$cus_address['zipcode']; ?></br>
...
<?php if($login_role!=2){ ?>
  <div class="col-6">
   <div style="border:1px solid black;border-radius:2%;margin:1%;padding: 2%;">
   <h4> Supplier detail </h4>
   <p>
     Name & ID : <?php echo $o['supllier_name']." / S".$o['seller_id']; ?></br>
     Mobile : <?php echo $o['supplier_mobile']; ?></br>
	</div>
    <div style="border:1px solid black;border-radius:2%;padding: 2%;">
   <b><h5>Supplier Settlement</h5>  </b></br>
   <table class="table ordertable"> 
      <tr><td>Product Price (Inc GST):</td><td><?php echo number_format($s_product_price,2); ?></td></tr>
      <tr><td> Tcs(1%):</td><td> <?php echo number_format($tcs,2); ?></td></tr>
      <tr><td>  Reseller Mantra Comission:</td><td>  <?php echo number_format($comission,2); ?></td></tr>
      <tr><td> Gst of Comission(18%):</td><td>  <?php echo number_format($gst,2); ?></td></tr>
      <tr><td> Penlty:</td><td>  <?php if($show_pen>0) {?><span style="color:red;"><?php echo "-".number_format($show_pen,2); ?></span><?php } else "0.00"; ?></td></tr>
      <tr class="lasttd"><td> Settlement Amount :</td><td> <?php echo number_format($settlement_amount,2); ?> </td></tr>
   </table>
  
   
	Rating : <?php echo $o['supplier_rating']; ?></br>
	Address :</br> <?php echo $s_address['name'].",".$s_address['contact']; ?></br>
	<?php echo $s_address['address'].",".$s_address['address2']; ?></br>
	<?php echo $s_address['city'].",".$s_address['state'].",".$s_address['zipcode']; ?></br>
	</div>
   </p>
</div><?php  } if($login_role!=3){?>
  <div class="col-6">
    <div style="border:1px solid black;border-radius:2%;margin:1%;padding: 2%;">
   <h4> Reseller detail </h4>
   <p>
     Name : <?php if(!$o['reseller_name']){ echo $o['reseller_fullname'];} else { echo $o['reseller_name'];} ?>/ R<?php echo $o['customer_id']; ?></br>
     Mobile : <?php echo $o['reseller_mobile']; ?></br>
	 </div>
     <div style="border:1px solid black;border-radius:2%;padding: 2%;">
     <b><h5>Reseller Settlement</h5>  </b></br>
	  <table class="table ordertable"> 
      <tr><td>Buyer Pay:</td><td> <?php echo number_format($buyer_pay,2); ?></td></tr>
      <tr><td> Product Price (Inc GST):</td><td> <?php echo number_format($r_product_price,2); ?></td></tr>
      <tr><td> Cod Charges:</td><td><?php echo number_format($cod_amount,2); ?> </td></tr>
      <tr><td> Shipping Charges:</td><td> <?php echo number_format($total_shipping,2); ?></td></tr>
      <tr><td> Coupon Applied:</td><td> <?php echo number_format($discount_amount,2); ?></td></tr>
      <tr class="lasttd"><td> Reseller Profit : </td><td> <?php echo number_format($my_earning,2); ?></td></tr>
   </table>
  ...
  <div class="row shop-tracking-status">
    ...
    <div class="col-md-12">
        <div class="well">
				...
				<?php if($o['awbNo']){ ?>
                <h4>Your order Tracking status: Awb No <?php echo $o['awbNo']; ?></h4>
				<p>Courier Name : <?php echo $o['carrierName']; ?></br>
				Track Url : <?php echo $o['track_url']; ?></br>
				</p>
				
				<?php 
				 $track_output=$o['track_status'];
				 if($track_output)
				{ ?>
				 <span class="label label-sm label-success">
									...
									<?php
														 ...
														 if($res['data']['events'])
														 {
															...
															echo $t_status=$events[0]['status'];
															?>
															</a></span> 
															<?php foreach($events as $e)
															{  ?>
															
															   <p style="color:black;"> Status: <?php echo $e['status']; ?></br>
															   Remarks: <?php echo $e['Remarks']; ?></br>
															   Location: <?php echo $e['Location']; ?></br>
															   Time: <?php echo date('d-m-Y h:i A',strtotime($e['Time'])); ?>
															   </p>
